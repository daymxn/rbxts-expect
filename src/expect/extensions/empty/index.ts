...
const baseMessage = new ExpectMessageBuilder(`Expected ${place.name} to ${place.not} be empty`).nestedMetadata({
...
function validateArrayIsEmpty(actual: defined[]) {
  ...
  return amount > 1
    ? message.suffix(`, but it had ${amount} elements`).fail()
    : message.suffix(`, but it had an element`).fail();
}
...
const empty: CustomMethodImpl = (source, actual) => {
  ...
  if (typeIs(actual, "table")) {
    ...
    return source.is_array || isArray(actual)
      ? validateArrayIsEmpty(actual as defined[])
      : validateObjectIsEmpty(actual);
